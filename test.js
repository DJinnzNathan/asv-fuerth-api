...
function test($, row, number, homeTeam) {
  const content = [];
  const teamClass = (homeTeam) ? '.bfv-matchdata-result__goals--team0' : '.bfv-matchdata-result__goals--team1';
  const goalData = $(teamClass);
  const temp = goalData[row];
  ...
  const values = [{ [character]: number }];
  const output = { [key]: values };
  content.push(output);
